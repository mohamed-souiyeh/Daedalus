...
export async function addCanvasEventListeners(canvas: HTMLCanvasElement) {
  ...
  canvas.addEventListener('mousedown', async function (event: MouseEvent) {
    ...
    if (globals.startAlgo === false && event.button === 0 && !(Math.floor((mouse.x - globals.gridOffsetLeft) / CELLSIZE) === globals.finish.x && Math.floor((mouse.y - globals.gridOffsetTop) / CELLSIZE) === globals.finish.y)) {
    ...
    if (globals.startAlgo === false && event.button === 2 && !(Math.floor((mouse.x - globals.gridOffsetLeft) / CELLSIZE) === globals.start.x && Math.floor((mouse.y - globals.gridOffsetTop) / CELLSIZE) === globals.start.y)) {
